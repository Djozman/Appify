...
func runCLI(args: CLIArgs) throws {
    ...
    guard let launcherBinary = Bundle.module.url(forResource: "Resources/Launcher", withExtension: nil) else {
        throw NSError(domain: "Appify", code: 1,
                      userInfo: [NSLocalizedDescriptionKey: "Launcher binary not found. Run './Scripts/install.sh' to rebuild."])
    }
    ...
    if let customIcon = setup.iconPath {
        ...
        if let (data, source) = FaviconFetcher.fetchWithSource(from: setup.url) {
            ...
            print(iconURL != nil ? "  Icon ready." : "  Conversion failed, continuing without icon.")
        }
    }

    print("  Building .app bundle...")
    let finalArgs = CLIArgs(url: setup.url, name: setup.name, width: setup.width,
